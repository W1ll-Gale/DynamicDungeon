...
namespace DynamicDungeon.Runtime.Nodes
{
    ...
    {
        private const string DefaultNodeName = "Simplex Noise";
        ...
        public SimplexNoiseNode(string nodeId, string outputChannelName, float frequency, float amplitude, Vector2 offset) : this(nodeId, DefaultNodeName, outputChannelName, frequency, amplitude, offset)
        {
        }

        public SimplexNoiseNode(string nodeId, string nodeName, string outputChannelName) : this(nodeId, nodeName, outputChannelName, 0.05f, 1.0f, Vector2.zero)
        {
        }

        public SimplexNoiseNode(string nodeId, string nodeName, string outputChannelName, float frequency, float amplitude, Vector2 offset)
        {
        ...
        [BurstCompile]
        internal struct SimplexNoiseJob : IJobParallelFor
        {
            ...
            public void Execute(int index)
            {
                ...
                float2 samplePosition = new float2(
                    (((float)x + Offset.x) * Frequency) + SeedOffset.x,
                    (((float)y + Offset.y) * Frequency) + SeedOffset.y);
