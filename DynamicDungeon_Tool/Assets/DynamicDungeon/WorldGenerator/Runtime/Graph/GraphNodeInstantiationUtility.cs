...
{
    public static class GraphNodeInstantiationUtility
    {
        ...
        public static bool TryCreateNodeInstance(GenNodeData nodeData, out IGenNode nodeInstance, out string errorMessage, bool allowPrototypeOutputFallback = false)
        {
            if (nodeData == null)
            {
                nodeInstance = null;
            ...
            if (!TryInstantiateNode(nodeType, nodeData, out nodeInstance, out errorMessage, allowPrototypeOutputFallback))
            {
                return false;
            }

            if (!TryApplyParameters(nodeInstance, nodeData, out errorMessage))
            {
                nodeInstance = null;
                return false;
            }
        ...
        public static bool TryInstantiateNode(Type nodeType, GenNodeData nodeData, out IGenNode nodeInstance, out string errorMessage, bool allowPrototypeOutputFallback = false)
        {
            ...
            for (constructorIndex = 0; constructorIndex < constructors.Length; constructorIndex++)
            {
                ...
                if (TryBindConstructor(constructors[constructorIndex], nodeData, allowPrototypeOutputFallback, out constructorMatch))
                {
            ...
            return TryInvokeConstructor(bestMatch, nodeType, "construction", out nodeInstance, out errorMessage);
        }

        public static bool TryInstantiatePrototypeNode(Type nodeType, string nodeId, string nodeName, out IGenNode nodeInstance, out string errorMessage)
        {
            ConstructorInfo constructor;
            object[] arguments;
            if (!TryGetBestPrototypeConstructor(nodeType, nodeId, nodeName, out constructor, out arguments))
            {
                nodeInstance = null;
            ...
            ConstructorMatch match = new ConstructorMatch(constructor, arguments, 0);
            return TryInvokeConstructor(match, nodeType, "prototype construction", out nodeInstance, out errorMessage);
        }

        public static bool TryGetBestPrototypeConstructor(Type nodeType, string nodeId, string nodeName, out ConstructorInfo constructor, out object[] arguments)
        {
            ConstructorInfo[] constructors = nodeType.GetConstructors(BindingFlags.Public | BindingFlags.Instance);
            ConstructorMatch bestMatch = null;
            ...
            {
                ConstructorMatch constructorMatch;
                if (TryBindPrototypeConstructor(constructors[constructorIndex], nodeType, nodeId, nodeName, out constructorMatch))
                {
                    if (bestMatch == null || constructorMatch.Score > bestMatch.Score)
                    {
                        bestMatch = constructorMatch;
        ...
        public static bool TryApplyParameters(IGenNode nodeInstance, GenNodeData nodeData, out string errorMessage)
        {
        ...
        public static bool IsGraphDataConstructorParameter(ParameterInfo parameter)
        {
            object unused;
            return TryGetSpecialArgumentValue(parameter, new GenNodeData(), out unused);
        }

        public static string SerialiseParameterValue(object value)
        {
            if (value == null)
            {
                return string.Empty;
        ...
        private static bool TryInvokeConstructor(ConstructorMatch match, Type nodeType, string operationName, out IGenNode nodeInstance, out string errorMessage)
        {
            try
            {
                nodeInstance = (IGenNode)match.Constructor.Invoke(match.Arguments);
                errorMessage = null;
                return true;
            }
            catch (TargetInvocationException exception)
            {
                ...
                nodeInstance = null;
                errorMessage = "Node type '" + nodeType.FullName + "' failed during " + operationName + ": " + innerException.Message;
                return false;
            }
            catch (Exception exception)
            {
                nodeInstance = null;
                errorMessage = "Node type '" + nodeType.FullName + "' failed during " + operationName + ": " + exception.Message;
                return false;
            }
        }

        private static bool TryBindConstructor(ConstructorInfo constructor, GenNodeData nodeData, bool allowPrototypeOutputFallback, out ConstructorMatch constructorMatch)
        {
            ...
            for (parameterIndex = 0; parameterIndex < parameters.Length; parameterIndex++)
            {
                ...
                {
                    arguments[parameterIndex] = allowPrototypeOutputFallback
                        ? CreatePrototypeVector2Value(parameter.Name ?? string.Empty)
                        : Vector2.zero;
                    continue;
                }

                object prototypeArgument;
                if (allowPrototypeOutputFallback &&
                    CanUsePrototypeArgumentForGraphData(parameter) &&
        ...
        private static bool TryBindPrototypeConstructor(ConstructorInfo constructor, Type nodeType, string nodeId, string nodeName, out ConstructorMatch constructorMatch)
        {
            ParameterInfo[] parameters = constructor.GetParameters();
            object[] arguments = new object[parameters.Length];
            int score = 0;
            ...
            for (parameterIndex = 0; parameterIndex < parameters.Length; parameterIndex++)
            {
                ...
                object prototypeArgument;
                if (TryGetPrototypeArgumentValue(nodeType, nodeId, parameter, out prototypeArgument))
                {
                    arguments[parameterIndex] = prototypeArgument;
                    continue;
                }
        ...
        private static float CreatePrototypeFloatValue(string parameterName)
        {
            ...
            if (parameterName.IndexOf("amplitude", StringComparison.OrdinalIgnoreCase) >= 0 ||
                parameterName.IndexOf("scale", StringComparison.OrdinalIgnoreCase) >= 0 ||
                parameterName.IndexOf("radius", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                return 1.0f;
            }
        ...
        private static int CreatePrototypeIntValue(string parameterName)
        {
            if (parameterName.IndexOf("octave", StringComparison.OrdinalIgnoreCase) >= 0 ||
                ...
                parameterName.IndexOf("logicalId", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                return 1;
            }
